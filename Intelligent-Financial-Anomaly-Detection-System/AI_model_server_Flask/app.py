...
def phone_lookup():
    ...
    try:
        ...
        def _not_found(msg="No AegisAI account linked to this number"):
            return jsonify({"found": False, "phoneNumber": phone, "message": msg}), 200
...
def _razorpay_client():
    ...
    try:
        ...
        raise ValueError("Razorpay package not installed. Run: pip install razorpay")
    load_dotenv()
...
# ══════════════════════════════════════════════════════════════════════════════
# BANK LINKING & NPCI SIMULATION
# ══════════════════════════════════════════════════════════════════════════════

import random, hashlib, time as _time

# ── BIN (Bank Identification Number) database for Indian cards ────────────────
_BIN_DB = {
    # HDFC Bank
    '404779': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Visa',       'type': 'Debit',  'color': '#004C8F'},
    '400081': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Visa',       'type': 'Debit',  'color': '#004C8F'},
    '437748': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Visa',       'type': 'Debit',  'color': '#004C8F'},
    '407387': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Visa',       'type': 'Debit',  'color': '#004C8F'},
    '452715': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Visa',       'type': 'Credit', 'color': '#004C8F'},
    '453101': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Visa',       'type': 'Credit', 'color': '#004C8F'},
    '517286': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'Mastercard', 'type': 'Debit',  'color': '#004C8F'},
    '607153': {'bank': 'HDFC Bank',             'upi_handle': 'hdfcbank', 'network': 'RuPay',      'type': 'Debit',  'color': '#004C8F'},
    # SBI
    '404099': {'bank': 'State Bank of India',   'upi_handle': 'sbi',      'network': 'Visa',       'type': 'Debit',  'color': '#22577A'},
    '404153': {'bank': 'State Bank of India',   'upi_handle': 'sbi',      'network': 'Visa',       'type': 'Debit',  'color': '#22577A'},
    '508526': {'bank': 'State Bank of India',   'upi_handle': 'sbi',      'network': 'RuPay',      'type': 'Debit',  'color': '#22577A'},
    '508527': {'bank': 'State Bank of India',   'upi_handle': 'sbi',      'network': 'RuPay',      'type': 'Debit',  'color': '#22577A'},
    '601200': {'bank': 'State Bank of India',   'upi_handle': 'sbi',      'network': 'RuPay',      'type': 'Debit',  'color': '#22577A'},
    '524148': {'bank': 'State Bank of India',   'upi_handle': 'sbi',      'network': 'Mastercard', 'type': 'Debit',  'color': '#22577A'},
    # ICICI Bank
    '421985': {'bank': 'ICICI Bank',            'upi_handle': 'icici',    'network': 'Visa',       'type': 'Debit',  'color': '#F7941D'},
    '437793': {'bank': 'ICICI Bank',            'upi_handle': 'icici',    'network': 'Visa',       'type': 'Debit',  'color': '#F7941D'},
    '400085': {'bank': 'ICICI Bank',            'upi_handle': 'icici',    'network': 'Visa',       'type': 'Debit',  'color': '#F7941D'},
    '430462': {'bank': 'ICICI Bank',            'upi_handle': 'icici',    'network': 'Visa',       'type': 'Credit', 'color': '#F7941D'},
    '544209': {'bank': 'ICICI Bank',            'upi_handle': 'icici',    'network': 'Mastercard', 'type': 'Credit', 'color': '#F7941D'},
    '607117': {'bank': 'ICICI Bank',            'upi_handle': 'icici',    'network': 'RuPay',      'type': 'Debit',  'color': '#F7941D'},
    # Axis Bank
    '400318': {'bank': 'Axis Bank',             'upi_handle': 'axisbank', 'network': 'Visa',       'type': 'Debit',  'color': '#800000'},
    '400384': {'bank': 'Axis Bank',             'upi_handle': 'axisbank', 'network': 'Visa',       'type': 'Debit',  'color': '#800000'},
    '415649': {'bank': 'Axis Bank',             'upi_handle': 'axisbank', 'network': 'Visa',       'type': 'Credit', 'color': '#800000'},
    '529086': {'bank': 'Axis Bank',             'upi_handle': 'axisbank', 'network': 'Mastercard', 'type': 'Debit',  'color': '#800000'},
    '607086': {'bank': 'Axis Bank',             'upi_handle': 'axisbank', 'network': 'RuPay',      'type': 'Debit',  'color': '#800000'},
    # Kotak
    '508533': {'bank': 'Kotak Mahindra Bank',   'upi_handle': 'kotak',    'network': 'Visa',       'type': 'Debit',  'color': '#EF3E23'},
    '508548': {'bank': 'Kotak Mahindra Bank',   'upi_handle': 'kotak',    'network': 'RuPay',      'type': 'Debit',  'color': '#EF3E23'},
    # Yes Bank
    '400649': {'bank': 'Yes Bank',              'upi_handle': 'yesbank',  'network': 'Visa',       'type': 'Debit',  'color': '#003087'},
    '508596': {'bank': 'Yes Bank',              'upi_handle': 'yesbank',  'network': 'RuPay',      'type': 'Debit',  'color': '#003087'},
    # PNB
    '508573': {'bank': 'Punjab National Bank',  'upi_handle': 'pnb',      'network': 'RuPay',      'type': 'Debit',  'color': '#F15A29'},
    '508574': {'bank': 'Punjab National Bank',  'upi_handle': 'pnb',      'network': 'RuPay',      'type': 'Debit',  'color': '#F15A29'},
    # Canara Bank
    '508571': {'bank': 'Canara Bank',           'upi_handle': 'canarabank','network': 'RuPay',     'type': 'Debit',  'color': '#006837'},
    # Bank of Baroda
    '508522': {'bank': 'Bank of Baroda',        'upi_handle': 'barodampay','network': 'RuPay',     'type': 'Debit',  'color': '#F26522'},
    # IDFC First
    '653236': {'bank': 'IDFC First Bank',       'upi_handle': 'idfcbank', 'network': 'Visa',       'type': 'Debit',  'color': '#1A3C6D'},
    '653237': {'bank': 'IDFC First Bank',       'upi_handle': 'idfcbank', 'network': 'Visa',       'type': 'Debit',  'color': '#1A3C6D'},
    # IndusInd
    '400342': {'bank': 'IndusInd Bank',         'upi_handle': 'indus',    'network': 'Visa',       'type': 'Debit',  'color': '#6B2D8B'},
    '430467': {'bank': 'IndusInd Bank',         'upi_handle': 'indus',    'network': 'Visa',       'type': 'Credit', 'color': '#6B2D8B'},
    # Union Bank
    '508513': {'bank': 'Union Bank of India',   'upi_handle': 'unionbank','network': 'RuPay',      'type': 'Debit',  'color': '#002060'},
    # Indian Bank
    '508530': {'bank': 'Indian Bank',           'upi_handle': 'indianbank','network': 'RuPay',     'type': 'Debit',  'color': '#005A9C'},
}

# All banks for NPCI simulation pool
_ALL_BANKS = [
    {'bank': 'HDFC Bank',            'upi_handle': 'hdfcbank',  'color': '#004C8F'},
    {'bank': 'State Bank of India',  'upi_handle': 'sbi',       'color': '#22577A'},
    {'bank': 'ICICI Bank',           'upi_handle': 'icici',     'color': '#F7941D'},
    {'bank': 'Axis Bank',            'upi_handle': 'axisbank',  'color': '#800000'},
    {'bank': 'Kotak Mahindra Bank',  'upi_handle': 'kotak',     'color': '#EF3E23'},
    {'bank': 'Yes Bank',             'upi_handle': 'yesbank',   'color': '#003087'},
    {'bank': 'Punjab National Bank', 'upi_handle': 'pnb',       'color': '#F15A29'},
    {'bank': 'Canara Bank',          'upi_handle': 'canarabank','color': '#006837'},
    {'bank': 'Bank of Baroda',       'upi_handle': 'barodampay','color': '#F26522'},
    {'bank': 'IDFC First Bank',      'upi_handle': 'idfcbank',  'color': '#1A3C6D'},
    {'bank': 'IndusInd Bank',        'upi_handle': 'indus',     'color': '#6B2D8B'},
    {'bank': 'Union Bank of India',  'upi_handle': 'unionbank', 'color': '#002060'},
]
...
@app.route('/api/card/bin', methods=['POST'])
def card_bin_lookup():
    """Look up bank info from first 6 digits of card (BIN)."""
    try:
        data = request.get_json() or {}
        card_number = str(data.get('cardNumber', '')).replace(' ', '')
        if len(card_number) < 6:
            return jsonify({'found': False, 'error': 'Need at least 6 digits'}), 400

        bin6 = card_number[:6]
        info = _BIN_DB.get(bin6)
        if info:
            return jsonify({'found': True, 'bin': bin6, **info})

        # Fallback: guess network from first digit
        first = bin6[0]
        network = 'Visa' if first == '4' else 'Mastercard' if first in ('5', '2') else 'RuPay' if first == '6' else 'Unknown'
        return jsonify({'found': False, 'bin': bin6, 'network': network,
                        'message': 'BIN not in database — card may still be valid'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def send_otp():
    """Generate and store OTP for mobile verification (demo mode — OTP returned in response)."""
    try:
        data = request.get_json() or {}
        mobile = str(data.get('mobile', '')).replace('+91', '').strip()
        if len(mobile) != 10 or not mobile.isdigit():
            return jsonify({'success': False, 'error': 'Invalid mobile number'}), 400

        otp = str(random.randint(100000, 999999))
        _otp_store[mobile] = {'otp': otp, 'expires': _time.time() + 600}  # 10-min TTL

        print(f"[AegisAI OTP] Mobile: {mobile} → OTP: {otp}")  # visible in Flask console

        return jsonify({
            'success': True,
            'message': f'OTP sent to +91 {mobile[:5]}XXXXX',
            # Return OTP in demo mode so frontend can auto-fill
            'demo_otp': otp,
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def verify_otp():
    ...
    try:
        ...
        if not record:
            return jsonify({'verified': False, 'error': 'OTP not sent or expired. Request a new one.'}), 400
        if _time.time() > record['expires']:
            del _otp_store[mobile]
            return jsonify({'verified': False, 'error': 'OTP expired. Request a new one.'}), 400
        if entered != record['otp']:
            return jsonify({'verified': False, 'error': 'Incorrect OTP'}), 400

        del _otp_store[mobile]  # single-use
        return jsonify({'verified': True, 'mobile': mobile})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/mobile/discover-banks', methods=['POST'])
def discover_banks():
    """
    NPCI simulation: given a mobile number, return banks where this mobile
    is registered for UPI. Uses deterministic selection so the same mobile
    always returns the same banks.
    """
    try:
        data = request.get_json() or {}
        mobile = str(data.get('mobile', '')).replace('+91', '').strip()
        ...
        # Deterministic bank selection from mobile number hash
        seed = int(hashlib.md5(mobile.encode()).hexdigest(), 16)
        rng  = random.Random(seed)
        count = rng.choice([1, 2, 2, 2, 3])  # 1–3 banks
        selected = rng.sample(_ALL_BANKS, count)

        banks = []
        for b in selected:
            masked_acc = 'XXXX XXXX ' + str(abs(hash(mobile + b['bank'])) % 10000).zfill(4)
            banks.append({**b, 'maskedAccount': masked_acc, 'ifsc': b['upi_handle'].upper() + '0000001'})

        return jsonify({'mobile': mobile, 'banks': banks, 'count': len(banks)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/vpa/lookup', methods=['POST'])
def vpa_lookup():
    """
    Resolve a UPI ID (VPA) to the account holder name.
    Tries Razorpay VPA validation, falls back to simulation.
    """
    try:
        data = request.get_json() or {}
        vpa  = str(data.get('vpa', '')).strip().lower()
        if '@' not in vpa:
            return jsonify({'found': False, 'error': 'Invalid UPI ID format'}), 400

        # Try Razorpay VPA validation
        ...
        if rzp_key and rzp_secret:
            try:
                ...
                result = client.payment.fetch_all({'vpa': vpa})
                if result and result.get('items'):
                    item = result['items'][0]
                    return jsonify({'found': True, 'vpa': vpa,
                                    'name': item.get('description', 'Account Holder'),
                                    'source': 'razorpay'})
            except Exception:
                pass  # Fall through to simulation

        # Simulation: derive name from VPA handle
        handle = vpa.split('@')[0]
        # Simulate a "name" from the handle (for demo)
        name_parts = [w.capitalize() for w in handle.replace('.', ' ').replace('_', ' ').split() if w.isalpha()]
        sim_name = ' '.join(name_parts[:2]) if name_parts else 'Account Holder'
        bank_handle = vpa.split('@')[1]
        bank_map = {h['upi_handle']: h['bank'] for h in _ALL_BANKS}
        bank_name = bank_map.get(bank_handle, bank_handle.upper())

        return jsonify({
            'found': True, 'vpa': vpa,
            'name': sim_name or 'Verified User',
            'bank': bank_name,
            'source': 'simulated',
            'verified': True,
        })
...
def link_bank_account():
    """Save linked bank account to Firestore user profile."""
    try:
        data = request.get_json() or {}
        uid       = data.get('uid', '')
        ...
        if not uid or not upi_id:
            return jsonify({'error': 'uid and upiId are required'}), 400
        ...
        try:
            ...
            db_fs.collection('users').document(uid).set({
                'phoneNumber': '+91' + mobile if not mobile.startswith('+') else mobile,
                'upiId':       upi_id,
                'linkedBank':  bank.get('bank', ''),
                'upiHandle':   bank.get('upi_handle', ''),
                'cardLast4':   card_last4,
                'accountLinked': True,
                'linkedAt':    fs.SERVER_TIMESTAMP,
            }, merge=True)
            return jsonify({'success': True, 'upiId': upi_id})
        except Exception:
            # Firebase not configured — return success anyway (demo)
            return jsonify({'success': True, 'upiId': upi_id, 'demo': True})
