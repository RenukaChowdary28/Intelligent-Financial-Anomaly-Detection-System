...
import MultiGatewayCheckout from './MultiGatewayCheckout';
  ...
  const loadPaymentHistory = async (userId) => {
    try {
      ...
      if (res.ok) {
        ...
        setPaymentHistory(MOCK_HISTORY);
      }
  ...
  const performPhoneLookup = async (phone) => {
    ...
    try {
      ...
      if (data.found) {
        ...
        // Backend says not found — try mock, then Firestore
        ...
        if (mock) {
          ...
          throw new Error('not_found');  // fall into catch → Firestore lookup
        }
      }
    } catch {
      // Network / server error — try mock, then Firestore direct lookup
      const mock = MOCK_RECIPIENTS[phone];
      if (mock) {
        const rd = { ...mock, phoneNumber: phone, found: true };
        setRecipient(rd);
        ...
        try {
          ...
          if (!snap.empty) {
            ...
            const rd = {
              phoneNumber: phone,
              upiId: u.upiId || `${(u.name || 'user').split(' ')[0].toLowerCase()}@yesbank`,
              displayName: u.name || u.email || 'AegisAI User',
              trustScore: 85,
              trustBadge: 'verified',
              found: true,
            };
