...
function RecurCard({ pay, onPause, onDelete, onSimulate }) {
...
export default function RecurringPayments() {
  ...
  const handleSimulate = async (pay) => {
    await Promise.all([
      updateDoc(doc(db, "recurringPayments", pay.id), {
        executionCount: (pay.executionCount || 0) + 1,
        lastExecuted: serverTimestamp(),
      }),
      addDoc(collection(db, "transactions"), {
        userId: user.uid,
        recipientUPI: pay.recipientUPI,
        recipientName: pay.recipientName || pay.recipientUPI,
    ...
    setSuccess(`Simulated ₹${parseFloat(pay.amount).toLocaleString("en-IN")} to ${pay.recipientName || pay.recipientUPI}`);
    setTimeout(() => setSuccess(""), 3000);
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <AnimatePresence>
                {payments.map(p => (
                  <RecurCard key={p.id} pay={p} onPause={handlePause} onDelete={handleDelete} onSimulate={handleSimulate} />
                ))}
              </AnimatePresence>
            </div>
          )}
