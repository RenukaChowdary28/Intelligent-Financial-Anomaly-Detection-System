...
import {
  TrendingDown, TrendingUp, CalendarClock, AlertTriangle,
  CheckCircle2, IndianRupee, RefreshCw, Info, Zap,
} from 'lucide-react';
import { Button } from '@/components/ui/button';

// ── Build 30-day projection ───────────────────────────────────────────────────
function buildForecast(recurringPayments, recentTxns) {
  // Estimate average daily spend from last 30 days of transactions
  const now = Date.now();
  const last30 = recentTxns.filter(t => {
    const ts = t.timestamp?.toDate?.() || new Date(t.timestamp || 0);
    return (now - ts.getTime()) < 30 * 86400000 && t.status === 'completed';
  });
    ...
    <div className="bg-gray-900 border border-gray-700 rounded-xl p-3 text-xs shadow-xl min-w-[160px]">
      ...
      <div className="space-y-1">
        <div className="flex justify-between gap-4">
          <span className="text-gray-500">Base spend</span>
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center">
      <div className="text-center space-y-3">
        <CalendarClock className="w-12 h-12 text-blue-400 mx-auto animate-pulse" />
        <p className="text-gray-400 text-sm">Building your 30-day forecast…</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Predictive spending trajectory for the next month</p>
          </div>
        </div>
        {user && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Avg Daily Spend',   value: `₹${fmt(dailyBase)}`,      Icon: TrendingDown, color: 'text-blue-400' },
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <p className="text-xs text-gray-500 mt-2 text-center">Cumulative net cash flow relative to today (negative = total spent)</p>
      </motion.div>
              ...
              d.scheduledItems.map((item, i) => (
                <div key={`${d.day}-${i}`}
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-gray-500 text-xs ml-2">{d.label}</span>
                  </div>
                </div>
              ))
            ))}
      ...
      {dangerDays === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Your spending pattern looks manageable over the next 30 days.</p>
          </div>
        </motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
        className="flex items-center gap-3 p-4 bg-gray-900 border border-gray-800 rounded-2xl text-xs text-gray-400">
