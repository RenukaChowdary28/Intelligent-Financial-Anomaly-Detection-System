...
import {
  Users, Plus, Trash2, Send, CheckCircle2,
  SplitSquareHorizontal, ChevronDown, ChevronUp,
} from "lucide-react";
...
export default function SplitBill() {
  ...
  const handleCalculate = () => {
    ...
    const perPerson = Math.floor((amt * 100) / valid.length) / 100;
    const remainder = Math.round((amt - perPerson * valid.length) * 100) / 100;
    const splits = splitType === "equal"
      ? valid.map((m, i) => ({
          name: m.name,
          upiId: m.upiId,
          amount: parseFloat((perPerson + (i === 0 ? remainder : 0)).toFixed(2)),
          paid: false,
        }))
      : valid.map(m => ({
          name: m.name,
