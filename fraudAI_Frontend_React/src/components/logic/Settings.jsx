...
import {
  ...
  Info, CheckCircle2, XCircle, Phone,
} from 'lucide-react'
...
const SettingsPage = () => {
  ...
  const handleSavePhone = async () => {
    ...
    if (trimmed && !/^\+91\d{10}$/.test(trimmed)) {
      setPhoneStatus("Enter a valid Indian mobile number starting with +91 (e.g. +917075084334).")
      return
    }
    setSavingPhone(true)
    setPhoneStatus("")
    try {
      ...
      await updateDoc(doc(db, "users", cu.uid), { phoneNumber: trimmed })
      setPhoneStatus("Phone number saved successfully.")
    } catch {
      setPhoneStatus("Failed to save phone number. Please try again.")
    } finally {
            ...
            <CardContent className="px-6 pb-6 -mt-8">
              ...
              <div className="mt-4 space-y-1.5">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {savingPhone ? "Saving…" : <><Save className="h-3.5 w-3.5 mr-1.5" />Save</>}
                  </Button>
                </div>
