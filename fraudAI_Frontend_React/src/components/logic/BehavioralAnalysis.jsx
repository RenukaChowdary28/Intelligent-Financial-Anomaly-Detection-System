...
function ResultBadge({ action }) {
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${ACTION_STYLE[action] || "bg-white/[0.07] text-slate-400 border-white/[0.09]"}`}>
      {action}
    </span>
  );
}
...
function AiSummary({ text }) {
  if (!text) return null;
  return <p className="text-xs text-slate-400 leading-relaxed border-t border-white/[0.07] pt-3 mt-3">{text}</p>;
}
    ...
    <Card className="bg-slate-900/80 border-white/[0.07]">
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">User ID</label>
            <Input value={form.user_id} onChange={e => setForm(f => ({ ...f, user_id: e.target.value }))} placeholder="user123" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Amount (₹)</label>
            <Input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="5000" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
        </div>
        ...
        {result && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2 pt-2 border-t border-white/[0.07]">
            ...
            {result.window_analysis?.filter(w => w.count_exceeded || w.amount_exceeded).map((w, i) => (
              <div key={i} className="text-xs bg-red-500/10 border border-red-500/20 rounded p-2">
                ...
                <span className="text-slate-300">{w.transaction_count} txns, ₹{w.total_amount?.toFixed(0)}</span>
              </div>
            ))}
...
function AmountPattern() {
  ...
  return (
    <Card className="bg-slate-900/80 border-white/[0.07]">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2 text-cyan-400"><DollarSign className="h-4 w-4" /> Amount Pattern</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid grid-cols-3 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">Amount (₹) *</label>
            <Input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="9999" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">User ID</label>
            <Input value={form.user_id} onChange={e => setForm(f => ({ ...f, user_id: e.target.value }))} placeholder="optional" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Avg Amount</label>
            <Input type="number" value={form.user_avg_amount} onChange={e => setForm(f => ({ ...f, user_avg_amount: e.target.value }))} placeholder="optional" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
        </div>
        {error && <p className="text-red-400 text-xs">{error}</p>}
      ...
      <CardContent className="space-y-3">
        <div className="flex gap-2">
          {["check", "register"].map(m => (
            <button key={m} onClick={() => setMode(m)}
              className={`text-xs px-3 py-1 rounded-full border transition-colors ${mode === m ? "bg-purple-600 border-purple-500 text-white" : "border-white/[0.09] text-slate-400"}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">User ID *</label>
            <Input value={form.user_id} onChange={e => setForm(f => ({ ...f, user_id: e.target.value }))} placeholder="user123" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Amount (₹)</label>
            <Input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} placeholder="2000" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Category</label>
            <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
              className="w-full bg-white/[0.07] border border-white/[0.09] text-white text-sm rounded-md px-2 py-1.5">
              {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div><label className="text-xs text-slate-400 block mb-1">Hour (0-23)</label>
            <Input type="number" min="0" max="23" value={form.hour} onChange={e => setForm(f => ({ ...f, hour: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
        </div>
        {error && <p className="text-red-400 text-xs">{error}</p>}
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">User ID *</label>
            <Input value={form.user_id} onChange={e => setForm(f => ({ ...f, user_id: e.target.value }))} placeholder="user123" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Amount (₹)</label>
            <Input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Device Trust (0-1)</label>
            <Input type="number" step="0.1" min="0" max="1" value={form.device_trust_score} onChange={e => setForm(f => ({ ...f, device_trust_score: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Distance (km)</label>
            <Input type="number" value={form.location_change_km} onChange={e => setForm(f => ({ ...f, location_change_km: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Hrs since login</label>
            <Input type="number" value={form.time_since_last_login_hrs} onChange={e => setForm(f => ({ ...f, time_since_last_login_hrs: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Failed Auth Attempts</label>
            <Input type="number" value={form.failed_auth_attempts} onChange={e => setForm(f => ({ ...f, failed_auth_attempts: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            ...
            <button key={field} onClick={() => toggle(field)}
              className={`text-xs py-1.5 rounded border transition-colors ${form[field] ? "bg-red-600 border-red-500 text-white" : "border-white/[0.09] text-slate-400"}`}>
    ...
    <Card className="bg-slate-900/80 border-white/[0.07]">
      ...
      <CardContent className="space-y-3">
        ...
        <button onClick={() => setForm(f => ({ ...f, is_merchant: !f.is_merchant }))}
          className={`text-xs px-3 py-1.5 rounded border transition-colors ${form.is_merchant ? "bg-cyan-600 border-cyan-500 text-white" : "border-white/[0.09] text-slate-400"}`}>
        ...
        {result && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2 pt-2 border-t border-white/[0.07]">
            ...
            <div className="flex gap-2 text-xs">
              <span className="text-slate-400">Confidence: <span className="text-white">{result.confidence}</span></span>
    ...
    <Card className="bg-slate-900/80 border-white/[0.07]">
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">User ID *</label>
            <Input value={form.user_id} onChange={e => setForm(f => ({ ...f, user_id: e.target.value }))} placeholder="user123" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">City</label>
            <Input value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} placeholder="Mumbai" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Latitude</label>
            <Input type="number" value={form.lat} onChange={e => setForm(f => ({ ...f, lat: e.target.value }))} placeholder="19.076" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Longitude</label>
            <Input type="number" value={form.lon} onChange={e => setForm(f => ({ ...f, lon: e.target.value }))} placeholder="72.877" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
        </div>
        {error && <p className="text-red-400 text-xs">{error}</p>}
...
function DeviceRisk() {
  ...
  return (
    <Card className="bg-slate-900/80 border-white/[0.07]">
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">Device ID *</label>
            <Input value={form.device_id} onChange={e => setForm(f => ({ ...f, device_id: e.target.value }))} placeholder="dev_abc123" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Days Since Install</label>
            <Input type="number" value={form.days_since_app_install} onChange={e => setForm(f => ({ ...f, days_since_app_install: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
        </div>
        <div className="grid grid-cols-3 gap-1.5">
          {boolFields.map(field => (
            <button key={field} onClick={() => toggle(field)}
              className={`text-xs py-1 rounded border transition-colors truncate ${form[field] ? "bg-orange-600 border-orange-500 text-white" : "border-white/[0.09] text-slate-400"}`}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2 pt-2 border-t border-white/[0.07]">
            ...
            {result.risk_flags?.map((f, i) => (
              <div key={i} className="text-xs bg-red-500/10 border border-red-500/20 rounded p-2">
                ...
                <span className="text-slate-400"> (-{f.points_deducted}pts): </span>
...
function RecipientTrust() {
  ...
  return (
    <Card className="bg-slate-900/80 border-white/[0.07]">
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div><label className="text-xs text-slate-400 block mb-1">Recipient UPI *</label>
            <Input value={form.recipient_upi} onChange={e => setForm(f => ({ ...f, recipient_upi: e.target.value }))} placeholder="john123@yesbank" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Verification</label>
          ...
          <div><label className="text-xs text-slate-400 block mb-1">Fraud Complaints</label>
            <Input type="number" value={form.fraud_complaints_against} onChange={e => setForm(f => ({ ...f, fraud_complaints_against: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Account Age (days)</label>
            <Input type="number" value={form.account_age_days} onChange={e => setForm(f => ({ ...f, account_age_days: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div><label className="text-xs text-slate-400 block mb-1">Past Txns Together</label>
            <Input type="number" value={form.past_successful_txns_with_user} onChange={e => setForm(f => ({ ...f, past_successful_txns_with_user: e.target.value }))} className="bg-white/[0.06] border-white/[0.08] text-white text-sm" /></div>
          <div className="flex items-end">
            <div className="flex gap-2 w-full">
              {[["0", "Not Blacklisted"], ["1", "Blacklisted"]].map(([v, l]) => (
                <button key={v} onClick={() => setForm(f => ({ ...f, recipient_blacklist_status: v }))}
                  className={`flex-1 text-xs py-1.5 rounded border transition-colors ${form.recipient_blacklist_status === v ? (v === "1" ? "bg-red-600 border-red-500 text-white" : "bg-green-600 border-green-500 text-white") : "border-white/[0.09] text-slate-400"}`}>
                  {l}
                </button>
              ))}
            </div>
        ...
        <button onClick={() => setForm(f => ({ ...f, is_first_time_recipient: !f.is_first_time_recipient }))}
          className={`text-xs px-3 py-1.5 rounded border transition-colors ${form.is_first_time_recipient ? "bg-yellow-600 border-yellow-500 text-white" : "border-white/[0.09] text-slate-400"}`}>
...
const SECTIONS = [
  ...
  { id: "amount", label: "Amount Pattern", icon: DollarSign, color: "text-cyan-400" },
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-6 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-2xl font-bold text-cyan-400 mb-1">Behavioral Analysis</h1>
            <p className="text-slate-400 text-sm mb-6">8 real-time behavioral fraud detection checks.</p>
          </motion.div>
            ...
            {SECTIONS.map((s) => (
              <button key={s.id} onClick={() => setOpen(s.id)}
                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-colors ${
                  open === s.id ? "bg-white/[0.07] border-gray-500 text-white" : "border-white/[0.07] text-slate-400 hover:border-gray-500"
                }`}>
                <s.icon className={`h-3.5 w-3.5 ${open === s.id ? s.color : "text-slate-500"}`} />
                {s.label}
              </button>
            ))}
          </div>
