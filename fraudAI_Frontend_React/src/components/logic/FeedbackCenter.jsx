      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-6 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-slate-400 text-sm mb-6">Submit human corrections to improve model accuracy.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <Input value={form.transaction_id} onChange={(e) => setForm(f => ({ ...f, transaction_id: e.target.value }))}
                      placeholder="e.g. txn_abc123" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-slate-400 block mb-1">Model Said</label>
                    ...
                    <div>
                      <label className="text-xs text-slate-400 block mb-1">Correct Label</label>
                      <div className="flex gap-2">
                        {["FRAUD", "LEGITIMATE"].map((v) => (
                  ...
                  <div>
                    <label className="text-xs text-slate-400 block mb-1">Analyst ID (optional)</label>
                    <Input value={form.analyst_id} onChange={(e) => setForm(f => ({ ...f, analyst_id: e.target.value }))}
                      placeholder="Your analyst ID" className="bg-white/[0.06] border-white/[0.08] text-white text-sm" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea value={form.notes} onChange={(e) => setForm(f => ({ ...f, notes: e.target.value }))}
                      ...
                      className="w-full bg-white/[0.07] border border-white/[0.09] text-white text-sm rounded-md p-2 resize-none h-20 focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                  </div>
                  ...
                  {submitResult && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      ...
                      <p className="text-slate-400 text-xs">{submitResult.message}</p>
                      <p className="text-slate-500 text-xs mt-1">Total feedback entries: {submitResult.total_feedback_entries}</p>
                    </motion.div>
                  )}
            ...
            <div className="space-y-4">
              <Card className="bg-slate-900/80 border-white/[0.07]">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base text-green-400 flex items-center justify-between">
                    ...
                    <button onClick={fetchStats} className="text-slate-400 hover:text-white">
                      <RefreshCw className="h-3.5 w-3.5" />
                    </button>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {statsLoading && <p className="text-slate-500 text-sm py-4 text-center animate-pulse">Loading stats…</p>}
                  {statsError && <p className="text-red-400 text-sm">{statsError}</p>}
                  {stats && !hasStats && (
                    <p className="text-slate-500 text-sm text-center py-4">No feedback submitted yet. Use the form to start.</p>
                  )}
                  {hasStats && (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          { label: "Total Feedback", value: stats.total_feedback, color: "text-cyan-400" },
                          ...
                          <div key={s.label} className="bg-white/[0.04] rounded-lg p-3">
                      ...
                      {/* AI Insight */}
                      {stats.ai_insight && (
                        <p className="text-xs text-slate-400 leading-relaxed border-t border-white/[0.07] pt-3">{stats.ai_insight}</p>
                      )}
                    </div>
                  )}
                </CardContent>
              ...
              {hasStats && stats.recent_corrections?.length > 0 && (
                <Card className="bg-slate-900/80 border-white/[0.07]">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-slate-300">Recent Corrections</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {stats.recent_corrections.slice(0, 5).map((c, i) => (
                      <div key={i} className="text-xs border-b border-white/[0.07]/50 pb-2 last:border-0 last:pb-0">
                        <div className="flex items-center justify-between">
                          <span className="text-slate-300 font-mono">{c.transaction_id}</span>
                        ...
                        {c.notes && <p className="text-slate-600 italic mt-0.5">{c.notes}</p>}
                      </div>
                    ))}
                  </CardContent>
                </Card>
