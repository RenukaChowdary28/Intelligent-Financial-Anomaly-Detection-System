...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-gray-800 border border-white/[0.07] rounded-lg px-3 py-2 text-xs">
          ...
          <div className="flex gap-2 flex-wrap mb-6">
            {PRESETS.map(p => (
              <button key={p.label} onClick={() => applyPreset(p)}
                className="px-3 py-1.5 text-xs rounded-full border border-white/[0.07] text-slate-400 hover:border-cyan-500/50 hover:text-cyan-400 hover:bg-cyan-500/5 transition-colors">
                {p.label}
              </button>
            ))}
          </div>
                ...
                <CardContent className="pt-4 space-y-4">
                  <div>
                    ...
                    <Input type="number" value={principal} onChange={e => setPrincipal(e.target.value)}
                      className="bg-white/[0.06] border-white/[0.08] text-white h-10 text-base" />
                ...
                <CardContent className="p-4 flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-slate-300">Principal: ₹{p.toLocaleString("en-IN")}</span>
                      <span className="text-xs text-slate-500 ml-auto">{(100 - interestPct)}%</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full bg-amber-500 flex-shrink-0" />
                      <span className="text-sm text-slate-300">Interest: ₹{interest.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</span>
                      <span className="text-xs text-slate-500 ml-auto">{interestPct}%</span>
                    </div>
                            ...
                            <thead className="bg-white/[0.04] sticky top-0">
                              <tr>
                                {["Month", "EMI (₹)", "Principal (₹)", "Interest (₹)", "Balance (₹)"].map(h => (
                                  <th key={h} className="px-4 py-2.5 text-left font-medium text-slate-300">{h}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {schedule.map((row, i) => (
                                <tr key={i} className={`border-t border-white/[0.07]/50 ${i % 2 === 0 ? "" : "bg-gray-800/30"}`}>
                                  <td className="px-4 py-2 text-slate-400">{row.month}</td>
                                  <td className="px-4 py-2 text-white font-medium">{row.emi.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</td>
                                  <td className="px-4 py-2 text-cyan-400">{row.principal.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</td>
                                  <td className="px-4 py-2 text-amber-400">{row.interest.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</td>
                                  <td className="px-4 py-2 text-slate-300">{row.balance.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
