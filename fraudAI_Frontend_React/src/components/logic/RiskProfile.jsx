...
function DimensionBar({ label, score, desc }) {
  ...
  return (
    <div className="p-3 bg-white/[0.03] rounded-lg">
      ...
      <div className="w-full h-1.5 bg-white/[0.07] rounded-full overflow-hidden mb-1">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-6 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-2xl font-bold text-cyan-400 mb-1">Your Risk Profile</h1>
          ...
          {loading && (
            <div className="flex items-center justify-center py-20">
              ...
              <span className="text-slate-400">Analysing your transaction history…</span>
            </div>
          )}
          ...
          {profile?.insufficient && (
            <Card className="bg-slate-900/80 border-white/[0.07]">
              <CardContent className="pt-8 pb-8 text-center">
                <TrendingUp className="h-10 w-10 text-cyan-400 mx-auto mb-3" />
                <p className="text-slate-300 font-semibold">Not enough transaction history</p>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
              ...
              <Card className="bg-slate-900/80 border-white/[0.07]">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm text-slate-300 flex items-center gap-2">
                    <Brain className="h-4 w-4 text-cyan-400" /> Behavioral Dimensions
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              {profile.trend.length > 0 && (
                <Card className="bg-slate-900/80 border-white/[0.07]">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-slate-300 flex items-center gap-2">
                      <TrendingUp className="h-4 w-4 text-cyan-400" /> Transaction Trend (last 30 days)
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card className={`border ${riskStyle.bg} bg-gray-800`}>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-slate-300 flex items-center gap-2">
                      <Brain className="h-4 w-4 text-cyan-400" /> AI Assessment
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-slate-300 leading-relaxed">{profile.assessment}</p>
