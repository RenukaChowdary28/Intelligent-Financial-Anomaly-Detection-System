  ...
  const initiatePayment = async () => {
    ...
    try {
      ...
      if (!orderRes.ok || order.error) {
        if (order.setup_required && window.Razorpay && RZP_KEY) {
          // Backend not configured — open Razorpay modal directly (client-side test mode)
          setStep('open');
          const testOptions = {
            key:         RZP_KEY,
          ...
          const rzpTest = new window.Razorpay(testOptions);
          rzpTest.on('payment.failed', (resp) => {
            setStep('failed');
    ...
    try {
      ...
      if (!verifyRes.ok) {
        if (result.setup_required) {
          // Backend can't verify — accept as cleared in test mode
          const fallback = {
